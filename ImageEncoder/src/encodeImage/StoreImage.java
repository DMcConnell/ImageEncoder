...
public class StoreImage
{
	public StoreImage(String path)
	{
		try
		{
			...
			new File("C:\\users\\duncanmcconnell\\desktop\\ImageEncryption").mkdir();
			File writeTo = new File("C:\\users\\duncanmcconnell\\desktop\\ImageEncryption\\text.txt");
			System.out.println("FIRST HALF WORKED");
			BufferedWriter writer = new BufferedWriter(new FileWriter(writeTo));
			for(int i = 0; i < rgbValues.length; i++)
			{
				writer.write(rgbValues[0]);
				writer.write("a");
				//writer.newLine();
			}
